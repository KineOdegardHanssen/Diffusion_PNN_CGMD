...
def ordered_packings(X,sigma):
    d,Lzs = X
    phi = 1 - (4./3*np.pi*(sigma**3)*101./(d**2*Lzs)) # 101 beads per chain
    tau = (3-phi)/2.
    return 1./tau
...
def notmonosph(X,sigma):
    d,Lzs = X
    phi = 1 - (4./3*np.pi*(sigma**3)*101./(d**2*Lzs))
    tau = 1./np.sqrt(phi)
    return 1./tau

def pshimd_spherepacking(X,sigma):
    d,Lzs = X
    phi = 1 - (4./3*np.pi*(sigma**3)*101./(d**2*Lzs))
    tau = phi**(-1/3.)
    return 1./tau

def overlapping_spheres(X,sigma):
    d,Lzs = X
    phi = 1 - (4./3*np.pi*(sigma**3)*101./(d**2*Lzs))
    tau = 1-np.log(phi)/2.
    return 1./tau

def overlapping_cylinders(X,sigma):
    d,Lzs = X
    phi = 1 - (4./3*np.pi*(sigma**3)*101./(d**2*Lzs))
    tau = 1-np.log(phi)
    return 1./tau
...
if big==False:
    ...
    plotname_cut_d = endlocation_static+'D_vs_d_dyn_vs_stat_vs_RWgeom_cut_norefl_rsphere'+str(rsphere)+'_modelr_findsigma_fittomid.png' 
else:
    ### d
...
plotname_panels_all = endlocation_static+'D_vs_d_norefl_rsphere'+str(rsphere)+'_allmodels_modelr_findsigma_fittomid_panels_all.png'
...
### powerlaw1(d,k,l,n)
if set_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw1, spacings_stat, Dparallel_stat, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw1, spacings_stat, Dparallel_stat, maxfev=thismaxfev, sigma=parstat_sigmas, absolute_sigma=True)
k_p1_stat        = popt[0]
l_p1_stat        = popt[1]
n_p1_stat        = popt[2]
rms_params       = np.sqrt(np.diag(pcov))
...
if set_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw1, spacings_dyn, Dparallel_dyn, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw1, spacings_dyn, Dparallel_dyn, maxfev=thismaxfev, sigma=pardyn_sigmas, absolute_sigma=True)
k_p1_dyn         = popt[0]
l_p1_dyn         = popt[1]
n_p1_dyn         = popt[2]
rms_params       = np.sqrt(np.diag(pcov))
...
#Perp
if set_sigma_errors==True:
    popt, pcov          = curve_fit(powerlaw1, spacings_stat, Dzs_stat, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov          = curve_fit(powerlaw1, spacings_stat, Dzs_stat, maxfev=thismaxfev, sigma=perpstat_sigmas, absolute_sigma=True)
...
if set_sigma_errors==True:
    popt, pcov            = curve_fit(powerlaw1, spacings_dyn, Dzs_dyn, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov            = curve_fit(powerlaw1, spacings_dyn, Dzs_dyn, maxfev=thismaxfev, sigma=perpdyn_sigmas, absolute_sigma=True)
...
##### powerlaw2(d,l,n)
if set_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw2, spacings_stat, Dparallel_stat, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw2, spacings_stat, Dparallel_stat, maxfev=thismaxfev, sigma=parstat_sigmas, absolute_sigma=True)
l_p2_stat        = popt[0]
n_p2_stat        = popt[1]
rms_params       = np.sqrt(np.diag(pcov))
...
if set_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw2, spacings_dyn, Dparallel_dyn, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov       = curve_fit(powerlaw2, spacings_dyn, Dparallel_dyn, maxfev=thismaxfev, sigma=pardyn_sigmas, absolute_sigma=True)
l_p2_dyn         = popt[0]
n_p2_dyn         = popt[1]
rms_params       = np.sqrt(np.diag(pcov))
...
#Perp
if set_sigma_errors==True:
    popt, pcov          = curve_fit(powerlaw2, spacings_stat, Dzs_stat, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov          = curve_fit(powerlaw2, spacings_stat, Dzs_stat, maxfev=thismaxfev, sigma=perpstat_sigmas, absolute_sigma=True)
...
if set_sigma_errors==True:
    popt, pcov            = curve_fit(powerlaw2, spacings_dyn, Dzs_dyn, maxfev=thismaxfev, sigma=cfsigmas, absolute_sigma=True)
if data_sigma_errors==True:
    popt, pcov            = curve_fit(powerlaw2, spacings_dyn, Dzs_dyn, maxfev=thismaxfev, sigma=perpdyn_sigmas, absolute_sigma=True)
...
ax.plot(fitds,Ds_op_dyn, '--', label=r'op')
ax.plot(fitds,Ds_hr_dyn, '-.', label=r'hr')
ax.plot(fitds,Ds_hc_dyn, ':', label=r'hc')
ax.plot(fitds,Ds_m1_dyn, '-.', label=r'c 1')
ax.plot(fitds,Ds_m2_dyn, '-.', label=r'c 2')
ax.plot(fitds,Ds_m3_dyn, '-.', label=r'c 3')
ax.plot(fitds,Ds_m4_dyn, '-.', label=r'c 4')
ax.plot(fitds,Ds_m5_dyn, '-.', label=r'c 5')
ax.plot(fitds,Ds_m6_dyn, '-.', label=r'c 6')
ax.plot(fitds,Ds_m7_dyn, '-.', label=r'c 7')
ax.plot(fitds,Ds_m8_dyn, '-.', label=r'c 8')
ax.plot(fitds,Ds_m9_dyn, ':', label=r'c 9')
#ax.plot(fitds,Ds_p1_dyn, '-.', label=r'Power law 1')
#ax.plot(fitds,Ds_p2_dyn, '-.', label=r'Power law 2')
ax.plot(fitds,Ds_p3_dyn, '--', label=r'pl')
ax.plot(fitds,Ds_p4_dyn, '-.', label=r'pl 2')
line1, = ax.plot(fitds,Ds_rm_dyn, '--,', label=r'rm')
...
ax.plot(fitds,Ds_op_stat, '--', label=r'op')
ax.plot(fitds,Ds_hr_stat, '-.', label=r'hr')
ax.plot(fitds,Ds_hc_stat, ':', label=r'hc')
ax.plot(fitds,Ds_m1_stat, '-.', label=r'c 1')
ax.plot(fitds,Ds_m2_stat, '-.', label=r'c 2')
ax.plot(fitds,Ds_m3_stat, '-.', label=r'c 3')
ax.plot(fitds,Ds_m4_stat, '-.', label=r'c 4')
ax.plot(fitds,Ds_m5_stat, '-.', label=r'c 5')
ax.plot(fitds,Ds_m6_stat, '-.', label=r'c 6')
ax.plot(fitds,Ds_m7_stat, '-.', label=r'c 7')
ax.plot(fitds,Ds_m8_stat, '-.', label=r'c 8')
ax.plot(fitds,Ds_m9_stat, ':', label=r'c 9')
#ax.plot(fitds,Ds_p1_stat, '-.', label=r'Power law 1')
#ax.plot(fitds,Ds_p2_stat, '-.', label=r'Power law 2')
ax.plot(fitds,Ds_p3_stat, '--', label=r'pl')
ax.plot(fitds,Ds_p4_stat, '-.', label=r'pl 2')
line1, = ax.plot(fitds,Ds_rm_stat, '--,', label=r'rm')
line1.set_dashes([2, 2, 10, 2])  # 2pt line, 2pt break, 10pt line, 2pt break
plt.xlabel(r'$d$ (nm)', fontsize=12)
plt.ylabel(r'$D/D_{\mathregular{bulk}}$', fontsize=12)
box = ax.get_position()
...
ax.plot(fitds,Ds_op_dyn_perp, '--', label=r'op') 
ax.plot(fitds,Ds_hr_dyn_perp, '-.', label=r'hr')
ax.plot(fitds,Ds_hc_dyn_perp, ':', label=r'hc')
ax.plot(fitds,Ds_m1_dyn_perp, '-.', label=r'c 1')
ax.plot(fitds,Ds_m2_dyn_perp, '-.', label=r'c 2')
ax.plot(fitds,Ds_m3_dyn_perp, '-.', label=r'c 3')
ax.plot(fitds,Ds_m4_dyn_perp, '-.', label=r'c 4')
ax.plot(fitds,Ds_m5_dyn_perp, '-.', label=r'c 5')
ax.plot(fitds,Ds_m6_dyn_perp, '-.', label=r'c 6')
ax.plot(fitds,Ds_m7_dyn_perp, '-.', label=r'c 7')
ax.plot(fitds,Ds_m8_dyn_perp, '-.', label=r'c 8')
ax.plot(fitds,Ds_m9_dyn_perp, ':', label=r'c 9')
#ax.plot(fitds,Ds_p1_dyn_perp, '-.', label=r'Power law 1')
#ax.plot(fitds,Ds_p2_dyn_perp, '-.', label=r'Power law 2')
ax.plot(fitds,Ds_p3_dyn_perp, '--', label=r'pl')
ax.plot(fitds,Ds_p4_dyn_perp, '-.', label=r'pl 2')
line1, = ax.plot(fitds,Ds_rm_dyn_perp, '--,', label=r'rm')
line1.set_dashes([2, 2, 10, 2])  # 2pt line, 2pt break, 10pt line, 2pt break
plt.xlabel(r'$d$ (nm)', fontsize=12)
plt.ylabel(r'$D/D_{\mathregular{bulk}}$', fontsize=12)
...
ax.plot(fitds,Ds_op_stat_perp, '--', label=r'op')
ax.plot(fitds,Ds_hr_stat_perp, '-.', label=r'hr')
ax.plot(fitds,Ds_hc_stat_perp, ':', label=r'hc')
ax.plot(fitds,Ds_m1_stat_perp, '-.', label=r'c 1')
ax.plot(fitds,Ds_m2_stat_perp, '-.', label=r'c 2')
ax.plot(fitds,Ds_m3_stat_perp, '-.', label=r'c 3')
ax.plot(fitds,Ds_m4_stat_perp, '-.', label=r'c 4')
ax.plot(fitds,Ds_m5_stat_perp, '-.', label=r'c 5')
ax.plot(fitds,Ds_m6_stat_perp, '-.', label=r'c 6')
ax.plot(fitds,Ds_m7_stat_perp, '-.', label=r'c 7')
ax.plot(fitds,Ds_m8_stat_perp, '-.', label=r'c 8')
ax.plot(fitds,Ds_m9_stat_perp, ':', label=r'c 9')
#ax.plot(fitds,Ds_p1_stat_perp, '-.', label=r'Power law 1')
#ax.plot(fitds,Ds_p2_stat_perp, '-.', label=r'Power law 2')
ax.plot(fitds,Ds_p3_stat_perp, '--', label=r'pl')
ax.plot(fitds,Ds_p4_stat_perp, '-.', label=r'pl 2')
line1, = ax.plot(fitds,Ds_rm_stat_perp, '--,', label=r'rm')
...
#ax.plot(fitds,Ds_p1__dyn, '-.', label=r'Power law 1')
#ax.plot(fitds,Ds_p2__dyn, '-.', label=r'Power law 2')
...
#ax.plot(fitds,Ds_p1__stat, '-.', label=r'Power law 1')
#ax.plot(fitds,Ds_p2__stat, '-.', label=r'Power law 2')
...
#ax3.plot(fitds,Ds_p2__dyn, '-.', label=r'Power law 2')
...
ax4.plot(fitds,Ds_op_stat_perp, '--', label=r'Ordered packings')
ax4.plot(fitds,Ds_hr_stat_perp, '-.', label=r'Hyperbola of revolution')
...
#ax.plot(fitds,Ds_p1__stat_perp, '-.', label=r'Power law 1')
ax4.plot(fitds,Ds_p3_stat_perp, '--', label=r'Power law')
#ax4.plot(fitds,Ds_p2__stat_perp, '-.', label=r'Power law 2')
...
plotname_difftime_sigma_free_massadj_dynamic_twoinone = endlocation + 'diffusiontime_dynamic_sigmafree_adjustingmass_twoinone.png'
...
bulkfilename_II = 'C:/Users/Kine/Documents/Projects_PhD/P2_PolymerMD/Planar_brush/Diffusion_bead_near_grid/D_vs_d/Bulk/Varysigmas/D_vs_sigma_better_rms_Nestimates10.txt'
...
plt.ylabel(r'$D_\perp/D_{\mathregular{bulk}}$', fontsize=12)#, dynamic brush', fontsize=12)
ax.yaxis.get_offset_text().set_fontsize(12)
...
for sigma in sigma_walker:
    ...
    line1, = ax1.plot(fitds_II,Ds_p3_stat_perp_II,label=r'$a$ = %s nm' %str(sigma), linewidth=2.0)
    line1.set_dashes(mylinestyles[j])
...
ax1.set_xlabel('d (nm)', fontsize=12)
ax1.set_ylabel(r'$D_\perp/D_{\mathregular{bulk}}$', fontsize=12)#, static brush', fontsize=12)
...
#plt.title('Net thickness %s nm, dynamic brush, adjusting mass' % str(thickness*1e9))
...
plt.xlabel('d (nm)', fontsize=12)
plt.ylabel('Diffusion time (s)', fontsize=12)
#plt.title('Net thickness %s nm, static brush, adjusting mass' % str(thickness*1e9))
plt.legend(loc='upper left', fontsize=11)
plt.tight_layout()
...
for i in range(Nsigmas):#(5):#
    line1, = ax2.plot(fitds_dyn_store[i],diffusiontimes_dyn[i],label=r'$a$ = %s nm' %str(sigma_walker[i]), linewidth=2.0)
    line1.set_dashes(mylinestyles[i])
ax2.set_xlabel(r'$d$ (nm)', fontsize=12)
ax2.set_ylabel('Diffusion time (s)', fontsize=12)
#plt.title('Net thickness %s nm, dynamic brush, adjusting mass' % str(thickness*1e9))
ax2.legend(loc="upper_right", fontsize=12)
#plt.legend(bbox_to_anchor=(1,0), loc="lower left", fontsize=12)
ax2.axis([1.98,10.02,-1e-9,0.37e-7])
ax.yaxis.get_offset_text().set_fontsize(12)
fig.tight_layout() #fig.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig(plotname_difftime_sigma_free_massadj_dynamic_twoinone)
'''
